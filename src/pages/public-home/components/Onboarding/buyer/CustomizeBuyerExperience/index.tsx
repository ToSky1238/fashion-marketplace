...
const CustomizeBuyerExperience: React.FC<FlowProps> = ({
  formik,
  handleNext,
  handleSkip,
}) => {
  ...
  const { updateState } = useBuyerMultiStepFormStore();
  ...
  return (
    <div className="w-full flex md:px-[32px] py-0 flex-col flex-shrink-0 justify-between items-center gap-[24px] h-full max-h-[600px]">
      ...
      <div className="flex gap-4">
        <button
          onClick={() => handleSkip && handleSkip(formik)}
          type="button"
...
export default CustomizeBuyerExperience;
