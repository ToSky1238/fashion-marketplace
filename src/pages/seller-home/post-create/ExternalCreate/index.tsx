...
import { ButtonText1 } from "common/styles/TextStyle";
...
export const ExternalCreate = () => {
  ...
  const mappedInitialValues: any = useMemo(() => {
    if (postItem) {
      return {
        ...
        externalName: data.externalName || postItem.details?.title || "",
        description:
        ...
        link:
          data.link ||
          (postItem.details &&
            "url" in postItem.details &&
            postItem.details.url) ||
          "",
        platformType:
          data.platformType ||
          (postItem.details &&
            "platform" in postItem.details &&
            postItem.details.platform) ||
  ...
  const handleSubmit = (
    ...
    const postType = PostType.EXTERNAL;
    const result: IExternalCreateData = {
      id: `p_${uuid()}`,
      type: PostType.EXTERNAL,
      title: data.externalName ?? "Default External Title",
      platform: data.platformType ?? "",
      url: data.link ?? "",
      description: data.description ?? "Default External Description",
      price: data.prices && data.prices.length > 0 ? data.prices[0] : 0,
      discount: data.discount ?? "",
      ...(id === undefined && { availability: true }),
    };
    if (result) {
      if (id) {
    ...
    <div className="w-full h-screen">
      ...
      <div className="max-w-5xl w-full">
        <Formik
          ...
          onSubmit={async (data) => {
            if (id) {
              dataDispatch({ data, type: "edit" });
            } else dataDispatch({ data, type: "push" });
                  ...
                  <TabPanel>
                    <PricingStep
                      ...
                      variants={(data.variants ?? []).filter(
                        (it: any): it is string => data.variants != null,
                      )}
                    />
                  </TabPanel>
                  <TabPanel>
              ...
              <div className="flex justify-between w-full my-8 px-2">
                ...
                <div className="flex gap-x-5">
                  {selectedTab !== forms.length - 1 ? (
                    <button className="items-center bg-blue-500 font-bold text-white py-2 px-4 rounded inline-flex">
                      <ButtonText1 as="button">Save & Next</ButtonText1>
                      <div className="text-secondaryContrast">
                    ...
                    <>
                      <button
                        type="button"
                        className="items-center border border-blue-500 text-blue-500 font-bold py-2 px-4 rounded"
                        onClick={async () => {
                      ...
                      <button
                        type="button"
                        className="inline-flex items-center bg-blue-500 font-bold text-white py-2 px-4 rounded"
                        onClick={async () => {
